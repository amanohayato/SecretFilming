...
MethodTimer::MethodTimer() :count_{ 0 }, c(&count_)
{
	Initialize();
}
...
void MethodTimer::AddEmpty(int size)
{
	...
	//何番目かを計算して
	int emptyCount = (int)emptyCount_.size() - 1;
	//識別用の空の関数を追加する
...
void MethodTimer::ToLastAction()
{
	...
	count_ = (int)targetFuncList_.size() - 1;
...
void MethodTimer::ToLast()
{
	if ((int)targetFuncList_.size() <= count_)return;

	count_ = (int)targetFuncList_.size();
