...
{
	int key = LoadGraph(filename.c_str());
	m_sprites[id].first = key;
	m_sprites[id].second.push_back(key);
	if (m_sprites[id].first == -1)
		throw std::string(filename + "の読み込みに失敗しました\n");
}
...
void Model::Load2D(const std::string& filename, const MODEL_ID& id, int allnum, const Point& splitnum, const Point& size)
{
	...
	m_sprites[id].first = LoadDivGraph(filename.c_str(), allnum, splitnum.x, splitnum.y, size.x, size.y, handle);
	for (int i = 0; i < allnum; ++i)
	{
		m_sprites[id].second.emplace_back(handle[i]);
	}
	delete[] handle;
}
...
void Model::DeleteAll()
{
	...
	while (mitr != m_models.end()) {
		MV1DeleteModel(mitr->second->GetHangle());
		++mitr;
	}
	m_models.clear();

	std::unordered_map<MODEL_ID, std::pair<int,std::vector<int>>>::iterator sitr = m_sprites.begin();
	while (sitr != m_sprites.end()) {
		//DeleteGraph(sitr->second.first);
		DeleteSharingGraph(sitr->second.first);
		++sitr;
	}
	m_sprites.clear();
}
...
{
	DrawBillboard3D(VGet(position.x, position.y, position.z), 0.5f, 0.5f, size, 0.0f, m_sprites[id].second[frame], true);
}
...
{
	DrawBillboard3D(VGet(position.x, position.y, position.z), origin.x, origin.y, size, 0.0f, m_sprites[id].second[frame], true);
}
...
{
	DrawBillboard3D(VGet(position.x, position.y, position.z), origin.x, origin.y, size, angle, m_sprites[id].second[frame], true);
}
...
{
	AlphaBlend(alpha);
	DrawBillboard3D(VGet(position.x, position.y, position.z), origin.x, origin.y, size, angle, m_sprites[id].second[frame], trans);
	DefaultBlend();
}
...
{
	AlphaBlend(alpha);
	DrawBillboard3D(VGet(position.x, position.y, position.z), origin.x, origin.y, size, angle, m_sprites[id].second[frame], trans, turn);
	DefaultBlend();
}
...
void Model::Draw2DBlend(const MODEL_ID& id, const Vector3& position, float size, int blend_mode, int alpha)
{
	...
	DrawBillboard3D(VGet(position.x, position.y, position.z), 0.5f, 0.5f, size, 0.0f, m_sprites[id].second[0], true);
...
{
	SetDrawBlendMode(blend_mode, alpha);
	DrawBillboard3D(VGet(position.x, position.y, position.z), origin.x, origin.y, size, angle, m_sprites[id].second[0], true);
	SetDrawBlendMode(BLEND_MODE::NoBlend, 0);
}
...
{
	SetDrawBlendMode(blend_mode, alpha);
	DrawBillboard3D(VGet(position.x, position.y, position.z), origin.x, origin.y, size, angle, m_sprites[id].second[0], true, turn);
	SetDrawBlendMode(BLEND_MODE::NoBlend, 0);
}
...
{
	SetDrawBlendMode(blend_mode, alpha);
	DrawBillboard3D(VGet(position.x, position.y, position.z), origin.x, origin.y, size, angle, m_sprites[id].second[frame], true, turn);
	SetDrawBlendMode(BLEND_MODE::NoBlend, 0);
}
...
int Model::GetHandle_Sprite(const MODEL_ID & id)
{
	return m_sprites.at(id).second.front();
}
...
bool Model::CheckASyncBillboard()
{
	...
	return std::all_of(m_sprites.begin(), m_sprites.end(),
		[] (const std::pair<MODEL_ID, std::pair<int,std::vector<int>>>& s)
	{
		return std::all_of(s.second.second.begin(), s.second.second.end(),
			[] (int handle)
		{
			return CheckHandleASyncLoad(handle) == FALSE;
		});
