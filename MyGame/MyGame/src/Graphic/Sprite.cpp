...
void Sprite::Load(const std::string& filename, const SPRITE_ID& id, int allnum, const Point& splitnum, const Point& size)
{
	...
	LoadDivGraph(filename.c_str(), allnum, splitnum.x, splitnum.y, size.x, size.y, handle);
...
void Sprite::DrawCircle(const SPRITE_ID & id, const Vector2 & position, double Percent, double StartPercent)
{
	DrawCircleGauge((int)position.x, (int)position.y, Percent, m_sprites[id], StartPercent);
}
...
void Sprite::Draw(int handle, const Vector2 & position, const Vector2 & origin, float alpha, const Vector2 & scale, float angle, bool trans, bool turn)
{
	AlphaBlend(alpha);
	DrawRotaGraph3((int)position.x, (int)position.y, (int)origin.x, (int)origin.y,
		(double)scale.x, (double)scale.y, (double)angle * PI / 180.0, handle, trans, turn);
