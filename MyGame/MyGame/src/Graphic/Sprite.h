...
public:
	...
	void Draw(int handle, const Vector2& position, const Vector2& origin = Vector2::Zero, float alpha = 1.0f, const Vector2& scale = Vector2::One, float angle = 0.0f, bool trans = true, bool turn = false);
	...
	///</summary>
	//円描画
	void DrawCircle(const SPRITE_ID& id, const Vector2& position, double Percent, double StartPercent);
	...
	int GetSliptFrameSize(const SPRITE_ID& id)const {
		...
		return (int)m_splitsprites.at(id).size();
	}
